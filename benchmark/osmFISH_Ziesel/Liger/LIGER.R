...
gene_count <- rowSums(Zeisel>0)
Zeisel <- Zeisel[gene_count >= 10,]
...
Gene_set <- intersect(rownames(osmFISH),rownames(Zeisel))

#### New genes prediction
Ligerex <- createLiger(list(SMSC_FISH = osmFISH, SMSC_RNA = Zeisel))
...
genes.leaveout <- intersect(rownames(osmFISH),rownames(Zeisel))
...
for(i in 1:length(genes.leaveout)) {
  print(i)
  start_time <- Sys.time()
  Ligerex.leaveout <- createLiger(list(SMSC_FISH = osmFISH[-which(rownames(osmFISH) %in% genes.leaveout[i]),], SMSC_RNA = Zeisel[rownames(osmFISH),]))
